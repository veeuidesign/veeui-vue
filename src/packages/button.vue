...
export default {
    ...
    props: {
        type: {
          ...
          default: '',
          validator(type) {
            if(type && !['warning', 'success', 'info', 'primary', 'danger'].includes(type)) {
                console.error(`type类型必须为${['warning', 'success', 'info', 'primary', 'danger'].join('、')}`)
            }
            return true;
          }
        },
        ...
        iconPosition: {
            ...
            validator(type) {
                if(!['left', 'right'].includes(type)) {
                    console.error(`icon-position属性必须为${['left', 'right'].join("或")}`)
                }
                return true
            }
        },
    ...
    computed: {
        btnClass() {
            let classes = [];
            if(this.type) {
                classes.push(`vee-button-${this.type}`)
            }
            if(this.iconPosition) {
                classes.push(`vee-button-${this.iconPosition}`)
            }
            return classes;
        }
    }
